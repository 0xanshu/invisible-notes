...
document
  .getElementById("wsRename")
  .addEventListener("click", () => openWorkspaceForm("rename"));
wsDeleteEl.addEventListener("click", () =>
  window.manager.deleteWorkspace(activeWorkspace),
);
document
...
wsFormEl.addEventListener("submit", (e) => {
  ...
  if (formMode === "create") window.manager.createWorkspace(name);
    ...
    window.manager.renameWorkspace(activeWorkspace, name);
  closeWorkspaceForm();
});
...
document
  ...
  .addEventListener("click", () => window.manager.newNote());
...
document.querySelectorAll("[data-theme-opt]").forEach((btn) => {
  btn.addEventListener("click", () =>
    window.manager.setTheme(btn.dataset.themeOpt),
  );
});
buildAccentDots();
...
document.getElementById("exportNotes").addEventListener("click", async () => {
  ...
  if (res && res.ok)
    flashStatus(`Exported ${res.count} note${plural(res.count)}`);
});
...
document.getElementById("importNotes").addEventListener("click", async () => {
  ...
  if (res && res.ok) {
    const dupes = res.skipped
      ? `, skipped ${res.skipped} duplicate${plural(res.skipped)}`
      : "";
...
function closeShortcuts() {
  ...
  if (focusBeforeShortcuts && focusBeforeShortcuts.isConnected)
    focusBeforeShortcuts.focus();
  focusBeforeShortcuts = null;
}

document
  .getElementById("shortcutsBtn")
  .addEventListener("click", openShortcuts);
